...
def get_ip_address(ifname='enp94s0f1'):
...
class CifarClient(fl.client.Client):
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        if self.force_update_next_round:
            # Send actual weights if previously forced
            ...
            # Insufficient improvement, will be decided by the server based on cache status
            print(f"Client {self.cid}: Insufficient improvement, reporting performance only.")
            weights_prime = []  # Empty list, server may decide to use cached weights
            self.no_update_this_round += 1
            self.force_update_next_round = True  # Ensure we send actual weights in the next round if needed
        else:
...
def main() -> None:
    ...
    trainset, testset = utils.load_cifar(data_dir=Path("/users/aga5h3"))
