...
class CustomFedAvg(FedAvg):
    ...
    def aggregate_fit(self, rnd: int, results: List[Tuple[ClientProxy, FitRes]], failures):
        ...
        # Remove the last 30% from the cache (not used in aggregation)
        unused_clients = performance_reports[int(0.7 * len(performance_reports)):]
        for report in unused_clients:
            unique_id = report["unique_id"]
...
def main():
    ...
    _, testset = utils.load_cifar(data_dir=Path("/users/aga5h3"))
