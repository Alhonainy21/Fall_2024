...
class CustomFedAvg(FedAvg):
    ...
    def aggregate_fit(self, rnd: int, results: List[Tuple[ClientProxy, FitRes]], failures):
        ...
        # Collect each client's weights and manage FIFO cache
        for client_proxy, fit_res in results:
            ...
            if fit_res.parameters.tensors:  # New update available
                ...
                # Cache new weights with FIFO handling
                if len(self.last_update_cache) >= max_cache_size:
                    removed_id, _ = self.last_update_cache.popitem(last=False)
                    print(f"Cache full. Removed oldest entry: Client {removed_id}")
                
                self.last_update_cache[unique_id] = fit_res.parameters  # Add new weights to cache
