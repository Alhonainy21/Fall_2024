...
def aggregate_fit(self, rnd: int, results: List[Tuple[ClientProxy, FitRes]], failures):
    ...
    # Collect and log each client's performance
    for client_proxy, fit_res in results:
        client_id = client_proxy.cid
    ...
    # Select the top 70% of clients based on validation accuracy
    ...
    for report in top_clients:
        ...
        if fit_res.parameters.tensors:
            ...
            print(f"No valid weights for client {client_proxy.cid} in round {rnd}.")

    # Perform weighted average only on selected clients
    if all_weights:
        num_layers = len(all_weights[0])
        aggregated_weights = [
            sum(weights[layer] for weights in all_weights) / total_data_points
