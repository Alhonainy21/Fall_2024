...
class CustomFedAvg(FedAvg):
    ...
    def aggregate_fit(self, rnd: int, results: List[Tuple[ClientProxy, FitRes]], failures):
        ...
        # Update the cache with only the top 70% of clients
        new_cache = {}
        for report in top_clients:
            ...
            if fit_res.parameters.tensors:
                print(f"Round {rnd}, Client {unique_id}: caching new weights.")
                new_cache[unique_id] = fit_res.parameters

        self.last_update_cache = new_cache

        # Aggregate weights for the top 70% clients
        for report in top_clients:
            unique_id = report["unique_id"]
            fit_res = report["fit_res"]
